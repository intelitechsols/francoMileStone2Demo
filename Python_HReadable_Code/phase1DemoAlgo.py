...
if arguments.output_folder is not None:
    if not os.path.exists(arguments.output_folder):
        ...
        raise Exception(arguments.output_folder + ' may exist but certainly  not as a directory')
...
if arguments.challenge:
    ...
    tl, br = (helper.array2integer(init_region[:2]), helper.array2integer(init_region[:2] + init_region[2:4]))

    try:
        Tracker.cmt_init(img_gray0, tl, br)
        while frame < num_frames:
            img = cv2.imread(images[frame])
            img_gray = cv2.cvtColor(im, cv2.COLOR_RGB2GRAY)
            Tracker.frame_analysis(img_gray)
    ...
    if arguments.vidpath is not None:
        ...
        # Otherwise assume it is a format string for reading images
            ...
            Capture = helper.VidCapFile(arguments.vidpath)
    ...
    if arguments.BoundBox is not None:
        ...
        BoundBox = helper.bbox2pts(BoundBox[None, :])
        ...
        # Get BBOX input 
        (tl, br) = helper.get_bbox(img_draw)
    ...
    while True:
        ...
        helper.project_keypoints(Tracker.tracked_keypts, img_draw, (0, 255, 0))
        ...
        if arguments.output_folder is not None:
            ...
            with open('{0}/keypoints_{1:08d}.csv'.format(arguments.output_folder, frame), 'w') as f:
                f.write('x y\n')
                np.savetxt(f, Tracker.tracked_keypts[:, :2], fmt='%.2f')
        ...
        print '{5:04d}: center: {0:.2f},{1:.2f} scale: {2:.2f}, active: {3:03d}, {4:04.0f}ms'.format(Tracker.center[0],
                                                                                                     Tracker.center[1],
                                                                                                     Tracker.scale_predict,
                                                                                                     Tracker.active_keypts.shape[
                                                                                                         0],
                                                                                                     1000 * (toc - tic),
                                                                                                     frame)
